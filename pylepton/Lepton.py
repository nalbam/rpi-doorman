...
class Lepton(object):
  ...
  """
    ...
    def capture(
        self, data_buffer=None, log_time=False, debug_print=False, retry_reset=True
    ):
    ...
    """
        ...
        if data_buffer is None:
            ...
            data_buffer.ndim < 2
            or data_buffer.shape[0] < Lepton.ROWS
            or data_buffer.shape[1] < Lepton.COLS
        ...
        while True:
            Lepton.capture_segment(
                self._handle, self._xmit_buf, self._msg_size, self._capture_buf[0]
            )
            if (
                retry_reset and (self._capture_buf[20, 0] & 0xFF0F) != 0x1400
        ...
        if debug_print:
            ...
            for i in range(Lepton.ROWS):
                ...
                fnum = fid & 0xFFF
                print(
                    "0x{0:04x} 0x{1:04x} : Row {2:2} : crc={1}".format(fid, crc, fnum)
                )
            print("---")
